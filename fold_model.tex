...
\[
  ...
  \quad u<0,\;c_3>0,
\]
with cubic phase and singular weight $\beta^{-1/2}$
truncated at scale $|u|^\alpha$.
The competition between the boundary singularity
and the cubic stationary point
induces a phase transition at $\alpha=\tfrac{1}{2}$.
We prove a uniform bound
$|\Phi_\alpha(u)|=O(|u|^{-1/2})$ in all regimes;
the bound is sharp ($\Theta(|u|^{-1/2})$) for $\alpha<\tfrac{1}{2}$,
while for $\alpha\ge\tfrac{1}{2}$ no limit amplitude exists
due to persistent interference between
endpoint and saddle contributions.
Earlier non-optimal decay exponents $|u|^{-1/6}$
and $|u|^{-1/3}$ are corrected.
\end{abstract}
...
\item \textup{(Sharpness, Region~I)}
  ...
  independent of $c_3$.
\item \textup{(Oscillatory saturation, Region~III)}
  ...
  \]
  in particular, $|u|^{1/2}|\Phi_\alpha(u)|$ does not converge.
\end{enumerate}
\end{theorem}
