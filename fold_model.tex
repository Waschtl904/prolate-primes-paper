...
\[
  ...
  u_n^{(\varphi)}\to-\infty.
\]
For $\varphi=\tfrac{\pi}{2}$: constructive interference,
$|u_n|^{1/2}|\Phi_\alpha(u_n)|\to 2\sqrt{\pi}$.
For $\varphi=\tfrac{3\pi}{2}$: destructive interference,
$|u_n|^{1/2}|\Phi_\alpha(u_n)|\to 0$.
These limits are asymptotic (not pointwise exact)
due to the $O(|u|^{-1})$ error in~\eqref{eq:rescaled}.
...
(inherently unstable due to phase cancellation),
$c_3=R_0=1$:
\begin{center}
...
regression in the presence of phase cancellation:
