...
$\beta^{-1/2}$ induces a phase transition at $\alpha=\tfrac{1}{2}$,
separating a classical Erd\'elyi regime from a CFU-type
coalescence regime.
We prove the uniform bound
\[
  |\Phi_\alpha(u)| = O(|u|^{-1/2}) \quad (u\to-\infty)
\]
for all $\alpha\ge 0$.
For $\alpha<\tfrac{1}{2}$ this decay is sharp
($\Theta(|u|^{-1/2})$) with leading constant $\sqrt{\pi}$.
For $\alpha\ge\tfrac{1}{2}$, the rescaled amplitude
$|u|^{1/2}|\Phi_\alpha(u)|$ does not admit a limit:
...
\item \textup{(Sharp regime, $\alpha<\tfrac{1}{2}$)}
  ...
  \]
\item \textup{(Non-convergent amplitude, $\alpha\ge\tfrac{1}{2}$)}
  \[
...
\medskip
\noindent\textbf{Structural necessity of CFU.}
The CFU reduction is essential only for $\alpha\ge\tfrac{1}{2}$,
where the stationary point collides with the singular endpoint.
In this regime, classical Erd\'elyi-type expansions fail
due to non-integrable interaction between phase curvature
and amplitude singularity; the CFU substitution
disentangles these effects and yields the correct scaling $\lambda\sim|u|^{3/2}$.
...
\begin{remark}[Proof of extremal oscillation]
\label{rem:extremal}
We justify the claims
\[
  \limsup_{u\to-\infty}|u|^{1/2}|\Phi_\alpha(u)| = 2\sqrt{\pi},
  \qquad
  \liminf_{u\to-\infty}|u|^{1/2}|\Phi_\alpha(u)| = 0.
\]
From~\eqref{eq:twoterm} and the coefficient computation
in Remark~\ref{rem:nolimit}:
\[
  |u|^{1/2}|\Phi_\alpha(u)|
  = \sqrt{\pi}
  \Bigl|e^{-i\pi/4}+e^{-2i\lambda(u)/3+i\pi/4}\Bigr|
  + O(|u|^{-1}),
  \qquad
  \lambda(u)=|u|^{3/2}(3c_3)^{-1/2}.
\]
The phase $\theta(u):=\tfrac{2}{3}\lambda(u)$
is continuous and strictly increasing with
$\theta(u)\to+\infty$ as $u\to-\infty$.
Hence $\theta(u)\bmod 2\pi$ is dense in $[0,2\pi)$;
no number-theoretic assumptions are required.
Consequently:
\begin{itemize}
\item There exist sequences $u_n\to-\infty$ with
  $\theta(u_n)\equiv \tfrac{\pi}{4}\pmod{2\pi}$,
  giving constructive interference:
  $|u_n|^{1/2}|\Phi_\alpha(u_n)|\to 2\sqrt{\pi}$.
\item There exist sequences $u_n\to-\infty$ with
  $\theta(u_n)\equiv \tfrac{\pi}{4}+\pi\pmod{2\pi}$,
  giving destructive interference:
  $|u_n|^{1/2}|\Phi_\alpha(u_n)|\to 0$.
\end{itemize}
