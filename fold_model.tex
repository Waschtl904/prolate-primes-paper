...
The CFU substitution disentangles these effects
and yields the correct large parameter $\lambda\sim|u|^{3/2}$.
...
is continuous, strictly increasing, and unbounded above
as $u\to-\infty$.
Hence its image contains every sufficiently large
interval $[T, T+2\pi]$;
in particular, for every $\varphi\in[0,2\pi)$
and every $\varepsilon>0$,
there exists $u$ such that
$\theta(u)\equiv\varphi\pmod{2\pi}$
up to $\varepsilon$.
Thus $\theta(u)\bmod 2\pi$ is dense in $[0,2\pi)$;
...
By density, there exist values of $u\to-\infty$
realising each target phase, giving:
\[
  \limsup_{u\to-\infty}|u|^{1/2}|\Phi_\alpha(u)|=2\sqrt{\pi},
  \qquad
  \liminf_{u\to-\infty}|u|^{1/2}|\Phi_\alpha(u)|=0.
