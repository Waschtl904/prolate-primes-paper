...
\item \emph{(Theorem~B.)} Under local quasimode hypotheses in the
  transition window, one obtains a conditional gap lower bound at the
  model scale.
\item \emph{(Corollary~C.)} Under Airy-type local spectral input,
...
sufficiently accurate transition-window quasimodes.}}
\end{center}
...
The latter becomes effective once Airy-type local spectral information is
available in the transition regime.
...
\item\label{Q4}
  the family is asymptotically orthogonal in the sense required to identify
  consecutive eigenvalues through the min--max principle.
\end{enumerate}
\end{assumption}
...
By \eqref{eq:quasimode} and the spectral theorem for compact self-adjoint
operators, the distance from $\mu_n(c)$ to the spectrum of $\Kc$ is at most
$\varepsilon(c)$.
Hence there exists an eigenvalue $\nu_n(c)\in\sigma(\Kc)$ such that
\[
  |\nu_n(c)-\mu_n(c)| \le \varepsilon(c).
\]
Likewise there exists $\nu_{n+1}(c)\in\sigma(\Kc)$ with
$|\nu_{n+1}(c)-\mu_{n+1}(c)|\le\varepsilon(c)$.
Under the asymptotic orthogonality and ordering hypothesis \ref{Q4},
these spectral points can be identified with the consecutive eigenvalues
$\lambdac{c}{n}$ and $\lambdac{c}{n+1}$.
Therefore
\[
  \gapc{c}{n}
...
\begin{assumption}[Airy transition input]
\label{ass:airy}
In the transition regime $n\sim 2c/\pi$, there exists a quasimode family as
in Assumption~\ref{ass:quasimode} with model spacing
...
Quasimode transfer & local residual control & $\delta_{\mathrm{model}}(c)$ & Abstract \\
...
explicit semiclassical quasimode problem.
