...
\begin{enumerate}[label=(\roman*)]
\item The RHP for $Y_c$ is fully specified by \eqref{eq:jump-matrix},
  the contour $\Sigma_c=[0,+\infty)$, and the normalisation $Y_c\to I$
  at infinity; see~\cite{ItsIzergin1990}.
...
Remark~\ref{rem:diagonal-control} together with uniform control of
derivatives and standard integrability; the off-diagonal Airy oscillation
...
\begin{enumerate}
\item Formulate and analyse the $2\times 2$ RH problem for
  $\widetilde{K}_c^{(\xi)}$ with jump \eqref{eq:jump-matrix} on
  $\Sigma_c=[0,+\infty)$ via IIKS~\cite{ItsIzergin1990}.
