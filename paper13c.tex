...
Moreover, there exists a tubular neighbourhood $\mathcal{U} \supset \Sigma_c^\pm$
such that the bound extends with uniform control of the first derivatives:
\[
  \bigl|\nabla_z \operatorname{Re}\!\bigl(2g_c(z) - \phi_c(z)\bigr)\bigr|
  \le C
\]
uniformly on $\mathcal{U} \setminus D_\epsilon$, for some constant $C>0$
independent of $c$.
\end{assumption}
...
The additional first-derivative control is needed for the Cauchy operator
estimates in the small-norm step (Proposition~\ref{prop:small-norm}).
\end{remark}
...
with uniform first-derivative control on a tubular neighbourhood,
where $\phi_c$ is the effective Widom edge phase of
\eqref{eq:effective-phase-def} restricted to the lens contours,
...
the first-derivative control on a tubular neighbourhood of the contour.
...
\]
on the lens contours $\Sigma_c^\pm \setminus D_\epsilon$, together
with uniform first-derivative control on a tubular neighbourhood.
\end{problem}
