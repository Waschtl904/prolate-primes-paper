...
analytic step.
Section~\ref{sec:rhp} provides the complete explicit RHP
(jump matrix, contour geometry, $g$-function ansatz, Airy
parametrix construction, and small-norm problem), identifying
precisely which analytic inputs are still required for a full proof.
...
\item \emph{(Section~\ref{sec:rhp}.)} Explicit RHP formulation:
  $g$-function ansatz, contour geometry, Airy parametrix,
  small-norm problem.
...
The explicit RHP section is a rigorous programme reduction:
all steps are constructed and the precise open inputs identified.
...
and provides in Section~\ref{sec:rhp} an explicit construction of all
steps of the Deift--Zhou steepest descent programme for this problem.
...
\begin{equation}\label{eq:jump-explicit}
  J_c(z)=I-2\pi i\,\vec{F}_c(z)\vec{G}_c(z)^\top
  =\begin{pmatrix}1-2\pi i & -2\pi i e^{2icz^{-1}}\\2\pi i e^{-2icz^{-1}} & 1+2\pi i\end{pmatrix}+O(1),
\end{equation}
...
\begin{equation}\label{eq:reconstruction}
  \widetilde K_c^{(\xi)}(x,y)
  =\frac{1}{2\pi i(x-y)}\,
  \bigl(Y_c^-(x)\bigr)_{12}\cdot\bigl(Y_c^-(y)\bigr)_{21}^{-1}.
\end{equation}
\end{definition}
...
\subsection{Step 2: $g$-function and contour deformation}

\begin{definition}[$g$-function]
\label{def:g-function}
The $g$-function for the sinc kernel at band edge $z=1$ (in original
coordinates) is defined by
\begin{equation}\label{eq:g-function}
  g_c(z) := \int_{-1}^{1}\log(z-t)\,\rho_c(t)\,dt,
  \quad z\in\mathbb{C}\setminus(-\infty,1],
\end{equation}
where $\rho_c(t)=\frac{1}{\pi}\sqrt{1-t^2}$ is the
equilibrium density for the sinc kernel support $[-1,1]$.
It satisfies:
\begin{enumerate}[label=(\roman*)]
\item $g_c(z)=\log z+O(z^{-1})$ as $z\to\infty$;
\item $g_c^+(t)+g_c^-(t)=2\log|t|+\pi i$, $t\in(-1,1)$ (variational
  identity);
\item $\operatorname{Re}(g_c^+(t)-g_c^-(t))=0$ for $t\in(-1,1)$
  (sign condition on the support).
\end{enumerate}
\end{definition}

\begin{definition}[Lens-opening contour and conjugated RHP]
\label{def:lens}
Set $\Phi_c^g(z):=c^{1/3}(z-1)^{2/3}$ (local conformal map near $z=1$
under edge scaling; see Lemma~\ref{lem:edge-normal-form}).
...
contours where $\operatorname{Re}(2g_c(z)-g_c^+(z)-g_c^-(z))<0$
...
\begin{remark}[$g$-function status]
\label{rem:g-status}
The $g$-function \eqref{eq:g-function} and the sign condition (iii) of
Definition~\ref{def:g-function} are the key analytic input that drives
the Deift--Zhou construction.
For the sinc kernel with equilibrium density
$\rho_c(t)=\frac{1}{\pi}\sqrt{1-t^2}$, the sign condition holds
on $(-1,1)$ and changes sign outside; this is the exact content of
Assumption~\ref{ass:branch} in the spectral variable.
\emph{Verifying the strict sign inequality uniformly in $c$ is a
remaining open input for the full proof.}
\end{remark}
...
\end{equation}
This matching requires the $g$-function sign condition
(Remark~\ref{rem:g-status}) and the explicit phase identification
...
If the $g$-function sign condition of Remark~\ref{rem:g-status} holds
uniformly in $c$, then:
\begin{enumerate}[label=(\roman*)]
...
Proposition~\ref{prop:small-norm} is conditional on the $g$-function
sign condition of Remark~\ref{rem:g-status}.
All other steps of the Deift--Zhou programme --- the IIKS construction,
...
The single remaining open input is:
\begin{center}
\fbox{\parbox{0.85\linewidth}{\centering\itshape
Uniform strict sign inequality
$\operatorname{Re}(g_c^+(z)-g_c^-(z))<0$ on $\Sigma_c^\pm\setminus D_\epsilon$,
for all sufficiently large $c$, with explicit constants.}}
\end{center}
\end{remark}
...
Conditionally on the $g$-function sign condition
(Remark~\ref{rem:g-status}):
\begin{enumerate}[label=(\roman*)]
...
verify the strict sign inequality of Remark~\ref{rem:what-is-open}
for the $g$-function \eqref{eq:g-function} uniformly in $c$.
...
\]
All steps except the $g$-function sign condition are explicit and complete.
\end{remark}
