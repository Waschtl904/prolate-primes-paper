...
\title[Operator Universality for the PSWF Transition Regime]{%
  ...
  A Fredholm--IIKS Reduction Framework with Explicit Analytic Gap}
...
The convergence of the IIKS vectors to their Airy limits, and the
resulting trace-class convergence $\norm{\widetilde{K}_c - K_{\mathrm{Ai}}}_{\mathfrak{S}_1}
\to 0$, are conditional on Conjecture~\ref{conj:airy-transition},
which identifies the Airy transition as the single remaining deep
analytic step.
A full proof of this conjecture requires Deift--Zhou steepest descent
on the Riemann--Hilbert problem associated to the rescaled sinc kernel.
...
\item \emph{(Theorem~I.)} Exact IIKS representation of the rescaled
  sinc kernel with explicit Airy limit conjecture.
\item \emph{(Corollaries A--C.)} Gap asymptotics, local counting,
  stable quasimodes; all conditional on Conjecture~\ref{conj:airy-transition}.
\end{itemize}
...
the convergence of the IIKS vectors of the rescaled sinc kernel to their
Airy counterparts in trace-class topology.
...
\item \emph{Scaling limit layer}: the IIKS vectors converge to Airy
  functions under the canonical edge-phase normalisation.
...
Its IIKS vectors are explicitly known.
Their convergence to Airy functions under edge-phase scaling is the
...
$K_{\mathrm{Ai}}$ \emph{if} the exponentials converge to Airy functions
(this convergence is Conjecture~\ref{conj:airy-transition}).
\end{lemma}
...
\begin{equation}\label{eq:iiks-vectors}
  \vec{F}_c(x)=\begin{pmatrix}e^{ic^{1/3}x}\\e^{-ic^{1/3}x}\end{pmatrix},\quad
  \vec{G}_c(y)=\begin{pmatrix}e^{-ic^{1/3}y}\\e^{ic^{1/3}y}\end{pmatrix},\quad
...
Part~(II) is formal: the convergence of the rescaled exponentials to Airy
functions after edge-phase normalisation is the content of
Conjecture~\ref{conj:airy-transition} and is \emph{not} proved here.
The distinction between these two layers is essential for the logical
consistency of the paper.
\end{remark}

\subsection{The Airy conjecture}

\begin{conjecture}[Airy transition of IIKS vectors]
\label{conj:airy-transition}
Let $\vec{F}_c$, $\vec{G}_c$ be the IIKS vectors of
Theorem~\ref{thm:emergent-iiks}.
Under the canonical edge-phase scaling of Lemma~\ref{lem:edge-normal-form},
\begin{equation}\label{eq:iiks-convergence}
  \vec{F}_c\to\vec{F}_\infty,\qquad
  \vec{G}_c\to\vec{G}_\infty
  \quad\text{in }L^2_{\mathrm{loc}}([0,\infty)),
\end{equation}
uniformly for $\xi$ in compact sets, as $c\to\infty$.
\end{conjecture}
...
\item Formulate the $2\times 2$ Riemann--Hilbert problem associated to
  the IIKS operator $\widetilde{K}_c^{(\xi)}$ via the Its--Izergin--%
  Korepin--Slavnov construction~\cite{ItsIzergin1990}.
...
\item Identify the Airy parametrix~\cite{TraWid1994} as the local model
  near the coalescing saddle.
\item Upgrade pointwise vector convergence to $\mathfrak{S}_1$-norm via
  the error chain of Section~\ref{sec:error-chain}.
\end{enumerate}
...
with Lemma~\ref{lem:hs}; the upgrade from $\mathfrak{S}_2$ to
$\mathfrak{S}_1$ uses Conjecture~\ref{conj:airy-transition} to control
the off-diagonal Airy oscillation.
\end{proof}
...
\begin{problem}[RH programme for Conjecture~\ref{conj:airy-transition}]
...
\item Identify the Airy parametrix~\cite{TraWid1994} at the coalescing
  saddle.
