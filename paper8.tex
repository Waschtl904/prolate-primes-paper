...
\item By the Hellmann--Feynman identity
\eqref{eq:HF} and the concentration estimate
of \cite{Osipov2013}, \S3.3:
$\Phi_n^{(c)}$ is supported in $[0,1]$
with $\|\Phi_n^{(c)}\|_{L^2([0,1])}^2
= \lambda_n^{(c)} \ge c_\kappa > 0$,
and the $t^2$-weight satisfies
\[
...
\;>\;0,
\]
since $t^2 \ge 1/4$ for $t \in [1/2, 1]$
and $\|\Phi_n^{(c)}\|_{L^2([1/2,1])}^2
\ge c_\kappa' > 0$
(mass concentration away from $0$,
\cite{Osipov2013}, Thm.~3.1).
Hence $|d\lambda_n^{(c)}/dc| \ge c_\kappa'' > 0$,
