...
\item \textbf{Theorem~\ref{thm:lower}
      ...
      (Lemma~\ref{lem:L2-mass}) with the Airy tail control
      to establish $L^2$-mass concentration on scale $c^{-1/2}$,
      and then derives the lower bound via a clean
...
together with Airy tail estimates.
...
Since $\tstar \in [-1,1]$,
and the Airy asymptotic \eqref{eq:airy-rep} yields
the tail estimate
\begin{equation}\label{eq:airy-tail}
  \int_{|t - \tstar| \ge R\,c^{-1/2}}
  |\Phihat_n^{(c)}(t)|^2\,dt
  \;\le\;
  c^{-1}\int_{|y| \ge R} |\Ai(y)|^2\,dy
  \cdot\bigl(1 + O(c^{-1/3})\bigr),
\end{equation}
and since $\int_R^\infty |\Ai(y)|^2\,dy \to 0$
as $R \to \infty$, we choose $R_0$ large enough
so that the right-hand side of \eqref{eq:airy-tail}
is at most $\tfrac{1}{8}$ for all $c \ge c_0$.

Combining \eqref{eq:mass-in-1} and \eqref{eq:airy-tail}:
\begin{equation}\label{eq:step6-final}
  \int_{|t - \tstar| \le R_0\,c^{-1/2}}
  |\Phihat_n^{(c)}(t)|^2\,dt
  \;\ge\;
...
\item Lemma~\ref{lem:L2-mass}
      ...
      \cite{BonamiKaroui2014} and Airy tail control)
...
contradicting the fixed lower mass from
Lemma~\ref{lem:L2-mass}.
\end{remark}
