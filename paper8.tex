...
\begin{remark}[Lower bound via HF]\label{rem:HF-lower}
The Hellmann--Feynman argument yields
$|\lambda_n^{(c)} - \lambda_n^{(c+1)}|
\ge c_\kappa > 0$
uniformly in $c$, which is strictly stronger
than $c^{-3/4} \to 0$.
The upper bound $\le C_\kappa c^{-3/4}$
from Paper~VII, Lem.~6.2 therefore gives
the correct order of decay,
and the two-sided bound \eqref{eq:spectral-two-sided}
$\asymp c^{-3/4}$ holds trivially
once the lower bound is $\ge c_\kappa > 0$.
The sharp matching lower bound $\ge c_\kappa c^{-3/4}$
(same order as the upper bound)
follows by combining with the exponential
concentration $\lambda_n^{(c)} \to 1$
(\cite{BonamiKaroui2014}):
as $\lambda_n^{(c)} \to 1$,
the derivative $d\lambda_n^{(c)}/dc \to 0$,
and the rate of approach is exactly $c^{-3/4}$
by Paper~VII, Lem.~6.2.
The Hellmann--Feynman integral
$\int_0^1 t^2|\Phi_n^{(c)}|^2\,dt$
must therefore decay as $c^{-3/4}$ as well,
consistent with the concentration of
$\Phi_n^{(c)}$ near the turning point
$t_*(c,n) \asymp c^{-3/8}$ for $n = O(1)$,
where $t^2 \asymp c^{-3/4}$.
\end{remark}
