...
\textbf{Step 1: Airy representation.}
By the composite Airy asymptotic expansion
(Theorem~A of Paper~VII, \cite{PaperVII}),
...
\begin{equation}\label{eq:airy-rep}
  ...
  \bigl(1 + O(c^{-1/3})\bigr),
\end{equation}
where the rescaled variable is
\[
  y(t) \;=\; \frac{c^{1/2}(t - \tstar)}{(\mu_n^{(c)})^{2/3}},
  \qquad
  c_\kappa \;\le\; \mu_n^{(c)} \;\le\; C_\kappa,
\]
and $\kappa_n^{(c)}$ is the normalisation coefficient
satisfying $|\kappa_n^{(c)}| \ge c_1 > 0$
