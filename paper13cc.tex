...
This is the mechanism that generates the Airy structure.
...
only after integrating the Widom edge phase in Lemma~\ref{lem:edge-normal-form}.
...
\end{equation}
which is an integrable rank-$2$ perturbation of the identity,
along the contour $\Sigma_c = [0,+\infty)$.

The contour $\Sigma_c=[0,+\infty)$ arises after edge scaling
and corresponds to the support of the limiting Airy operator.
It is inherited from the Fourier integral representation of the sinc
kernel after the substitution $\eta = c + c^{1/3}u$ maps the
frequency interval $[-c,c]$ to the half-line $u \in (-\infty,0]$,
which in rescaled spatial coordinates becomes $[0,+\infty)$.
A complete justification of this contour reduction is part of
Conjecture~\ref{conj:airy-transition} itself.
...
and standard integrability (guaranteed by Remark~\ref{rem:uniform-integrability});
the off-diagonal oscillation is controlled
...
Motivated by equilibrium measure considerations for orthogonal polynomial
ensembles supported on $[-1,1]$ (see~\cite{Deift1999}), we introduce the
following \emph{candidate $g$-function}:
\[
  g_c(z) := \int_{-1}^{1} \log(z-t)\,\rho(t)\,dt,
  \quad z\in\mathbb{C}\setminus(-\infty,1],
\]
where $\rho(t)=\frac{1}{\pi}\sqrt{1-t^2}$ is the Wigner semicircle density
on $[-1,1]$.
The branch of $\log(z-t)$ is fixed by continuity from $\Im z>0$.

This choice is not canonically determined by the prolate problem alone;
it is motivated by the requirement that it yields the correct factorisation
of the jump matrix $J_c(z)$ and that the resulting phase $2g_c - \phi_c$
satisfies Assumption~\ref{ass:g-sign}. The validity of this choice
is part of the analytic content of Assumption~\ref{ass:g-sign}.
\end{definition}

\begin{remark}[Status of the $g$-function candidate]
\label{rem:g-candidate-status}
The candidate $g_c$ of Definition~\ref{def:g-function} is a motivated
hypothesis, not a derived object. In the classical orthogonal polynomial
setting, the $g$-function is uniquely determined as the Stieltjes transform
of the equilibrium measure, and the real-part matching condition
$\operatorname{Re}(g_c^+ + g_c^-) = \operatorname{Re}(\phi_c)$ on $(-1,1)$
is then an \emph{identity} following from the Euler--Lagrange equations
of the associated energy functional.
In the present setting, $\phi_c$ is the Widom edge phase (extrinsic to the
prolate operator's OP structure) and $g_c$ is chosen a priori.
Verifying condition~(i) of Assumption~\ref{ass:g-sign} is therefore
a \emph{nonlinear matching problem}: one must confirm that
$\operatorname{Re}(g_c^+(t) + g_c^-(t)) = \operatorname{Re}(\phi_c(t))$
holds on $(-1,1)$, which is equivalent to asking whether the semicircle
equilibrium measure correctly reproduces the Widom phase. This is not
automatic, and constitutes a substantial part of the difficulty of
...
phase $\phi_c$ of \eqref{eq:effective-phase-def}, for all sufficiently
large $c$:

\begin{enumerate}[label=(\roman*)]
\item \textup{(Real-part matching on the support.)}
The candidate $g_c$ yields the correct real-part matching required
for the LU-factorisation of the jump matrix $J_c(z)$ on the support
...
\[
  ...
  \quad t \in (-1,1).
\]
The imaginary part of the jump is automatically correct by
Proposition~\ref{prop:g-properties}(iii), so that condition~(i)
is the only nontrivial compatibility requirement for the
conjugated jump $J_c^T$ to be of oscillatory off-diagonal form.
As noted in Remark~\ref{rem:g-candidate-status}, this condition
encodes a nonlinear compatibility between the semicircle equilibrium
potential and the Widom phase, and is not automatic.
...
$\mathcal{U} \supset \Sigma_c^\pm$ there exists a constant $C > 0$,
...
which in turn depends on the real-part matching of condition~(i).
Together they constitute a single coupled condition on the pair
$(g_c, \Sigma_c^\pm)$.
\end{assumption}
...
(corresponding to $z=0$ in rescaled coordinates).
The additional first-derivative control in condition~(iii) is needed
for the Cauchy operator estimates in the small-norm step
(Proposition~\ref{prop:small-norm}); the uniform Lipschitz requirement
on $\Sigma_c^\pm$ ensures that the associated singular integral
operator $C_{\Sigma_c^\pm}$ is bounded on $L^2(\Sigma_c^\pm)$
uniformly in $c$, by standard Muckenhoupt-type estimates.
\end{remark}
...
More precisely:
\begin{enumerate}[label=(\roman*)]
\item Condition~(i) ensures that the conjugated jump $J_c^T$ is of
  oscillatory off-diagonal form, enabling the LU-factorisation and
  lens-opening construction of Definition~\ref{def:lens}.
\item Conditions~(ii) and~(iii) together imply the small-norm bound
  \eqref{eq:Rc-jump-bound} of Proposition~\ref{prop:small-norm},
  giving $R_c = I + O(c^{-1/3})$.
\item Combined with the Airy parametrix of Definition~\ref{def:airy-parametrix}
  and the reconstruction stability of Remark~\ref{rem:reconstruction-stability},
...
cut along the negative real axis, consistent with the standard Airy
parametrix construction~\cite{Deift1999}.
The parametrix is built explicitly from the Airy function $\Ai$
and its derivatives:
...
strict sign inequality $\operatorname{Re}(2g_c-\phi_c) < -\kappa < 0$
...
The passage from RHP convergence to kernel convergence in
Proposition~\ref{prop:kernel-from-rhp}(i) uses the following
stability property of the IIKS reconstruction map
\eqref{eq:reconstruction}: the map
\[
  Y_c \;\longmapsto\;
  K_c(x,y) = \frac{1}{2\pi i(x-y)}\bigl(Y_c^-(x)^{-1}Y_c^-(y)\bigr)_{21}
\]
is continuous with respect to uniform matrix convergence on
compact subsets of $\mathbb{C}\setminus\Sigma_c$.
More precisely, if $\|Y_c - Y_{\mathrm{Ai}}\|_{L^\infty(\partial D_\epsilon)}
= O(c^{-1/3})$, then the bilinearity of the reconstruction formula
and the uniform invertibility of $Y_c^-$ (which follows from the
small-norm bound \eqref{eq:Rc-norm} and the explicit invertibility
of $P^{(\mathrm{Ai})}$) give the pointwise kernel bound directly.

The subsequent upgrade to trace-class convergence in part~(ii) requires
additionally:
\begin{itemize}
\item \emph{Bounded Cauchy operators:}
  the Cauchy singular integral operator $C_{\Sigma_c^\pm}$ is bounded
  on $L^2(\Sigma_c^\pm)$ uniformly in $c$; this follows from
  Assumption~\ref{ass:g-sign}(iii), which ensures that the lens
  contours $\Sigma_c^\pm$ are uniformly Lipschitz with constants
  independent of $c$, enabling standard Muckenhoupt-type estimates
  for singular integrals on Lipschitz curves;
\item \emph{Resolvent stability:}
  the operator $(I - C_{w_c})$ is invertible with uniformly bounded
  inverse on $L^2(\Sigma_c^\pm)$, where $C_{w_c}$ is the weighted
  Cauchy operator associated to $w_c := J_{R_c} - I$; this follows
  from the small-norm bound \eqref{eq:Rc-jump-bound}.
\end{itemize}
Both ingredients are supplied by Assumption~\ref{ass:g-sign} and
Proposition~\ref{prop:small-norm}. The present non-OP setting
requires these steps to be verified directly, without recourse to
orthogonal polynomial machinery.
\end{remark}
...
confirm that the candidate $g_c$ of Definition~\ref{def:g-function}
satisfies
$\operatorname{Re}(g_c^+(t) + g_c^-(t)) = \operatorname{Re}(\phi_c(t))$
for $t \in (-1,1)$, so that the conjugated jump $J_c^T$ is of
oscillatory off-diagonal form;
...
chosen to be uniformly Lipschitz in $c$ with constants independent of $c$;
\item \emph{Strict inequality with derivative control:}
establish the uniform bound
$\operatorname{Re}(2g_c(z)-\phi_c(z)) < -\kappa < 0$
on $\Sigma_c^\pm \setminus D_\epsilon$, uniformly in $c$, together with
the gradient bound $|\nabla_z \operatorname{Re}(2g_c - \phi_c)| \le C$
on a tubular neighbourhood.
\end{enumerate}
These three conditions are coupled: the choice of contour depends on
the sign of $2g_c - \phi_c$, which in turn depends on the real-part
matching of~(i). Once all three are established,
Proposition~\ref{prop:small-norm} gives $R_c = I + O(c^{-1/3})$,
...
\]
All steps except Assumption~\ref{ass:g-sign}(i)--(iii) are explicit
and complete. Remark~\ref{rem:reconstruction-stability} makes the
passage $Y_c \to K_c$ rigorous without OP machinery, using the
uniform Lipschitz property of the lens contours.
The completeness of the reduction --- that Assumption~\ref{ass:g-sign}
alone suffices --- is the content of
...
(Lemma~\ref{lem:edge-normal-form}), not from the Fourier representation
of the kernel itself --- a key structural distinction.
...
in one well-posed coupled condition
(Proposition~\ref{prop:reduction-completeness}).
\end{enumerate}
...
\label{prob:main}
Verify Assumption~\ref{ass:g-sign} in its three coupled components:
the real-part matching $\operatorname{Re}(g_c^+ + g_c^-) = \operatorname{Re}(\phi_c)$
...
This problem appears amenable to detailed local analysis near the
edge (corresponding to $z=0$ in rescaled coordinates),
