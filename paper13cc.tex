...
accumulated from the spectral density near the edge. The integration
step is the precise birthplace of the Airy structure.
...
Widom edge phase.
The interaction of these two objects --- which are not independently
specified but coupled through the nonlinear constraint of
Assumption~A --- is the essential difficulty of the whole problem.
\end{remark}
...
geometric conditions. We emphasise that these conditions are standard
in scope but nontrivial in practice: without explicit control of $g_c$
(which requires Assumption~A first), the sign condition and Lipschitz
estimates on $\Sigma_c^\pm$ cannot be verified directly.
...
These conditions are standard in the Deift--Zhou framework but
remain nontrivial here, as the lack of explicit control of $g_c$
(prior to establishing Assumption~A) prevents a direct verification.
...
It is standard in scope within the Deift--Zhou framework, but
nontrivial in practice due to the lack of explicit control of $g_c$
prior to establishing Assumption~A.
...
$A_2$ saddle once Assumption~A is established, but this has not
been verified.
\end{remark}
...
Standard in scope but nontrivial without explicit control of $g_c$.}}
\end{center}
\end{remark}
...
analysis near $z=0$ once Assumption~A is established, but
this verification is nontrivial and has not been carried out.
...
(Muskhelishvili~\cite{Muskhelishvili1953}), iterating to a
fixed point. Existence of a fixed point is the central open question.
...
exponential decay required for the small-norm argument, but remains
nontrivial without explicit control of $g_c$.
...
of the kernel itself --- a key structural distinction that explains
why Airy universality is a global RHP phenomenon and cannot be
seen locally.
...
be established first; Assumption~B (geometric) is then the
expected next step, standard in scope but nontrivial in practice.
\end{enumerate}
