...
compatible with the Widom edge phase via a scalar
Riemann--Hilbert matching condition with nonlinear coupling.
Given such a $g$-function, all steepest-descent geometry (sign
condition, Lipschitz contours, no additional critical points)
reduces to a second, purely analytic assumption.
...
equivalent to a single variational compatibility problem, whose
solvability is currently open.
\end{abstract}
...
with the Widom edge phase, characterised by a scalar
Riemann--Hilbert equation with nonlinear coupling.
The second is geometric: given such a $g$-function, the
steepest-descent contours carry the required sign and regularity
conditions.
...
whose solvability is currently open
...
More precisely:
\begin{enumerate}[label=(\roman*)]
\item \textup{(Sufficiency.)} Assumptions~A and~B imply
...
\item \textup{(Converse, heuristic.)} If Airy universality holds
  in the sense of \eqref{eq:fredholm-universality}, then the
  associated RHP solution $Y_c$ is expected to converge to the
  Airy parametrix, which necessitates a function playing the role
  of $g_c$ and satisfying \eqref{eq:g-matching} in the sense of
  Assumption~A, together with the steepest-descent geometry of
  Assumption~B. A rigorous converse is not established here.
\end{enumerate}
...
be found as the solution of a compatibility equation between two
a priori unrelated objects: the equilibrium potential and the Widom
edge phase.
...
This can be viewed as a scalar Riemann--Hilbert problem with
nonlinear coupling through the real-part constraint: given the
boundary datum $\operatorname{Re}(\phi_c(\cdot))$ on $(-1,1)$
--- which depends on $g_c$ implicitly through the Widom phase ---
find an analytic function with the prescribed real boundary values
\eqref{eq:core-problem} and logarithmic growth at infinity.
The coupling between $g_c$ and $\phi_c$ renders this a genuinely
nonlinear problem, distinct from the classical linear scalar
RHP arising in the OP setting.

Unlike the classical OP setting, where $\phi_c$ is the equilibrium
potential of a known measure and \eqref{eq:core-problem} holds
automatically, the Widom phase is extrinsic and \eqref{eq:core-problem}
...
is formulated as an open problem, and its resolution is the
subject of future work.
\end{problem}

\begin{remark}[Hilbert transform formulation]
\label{rem:hilbert-formulation}
In the regime where $\phi_c$ can be treated as a given external datum
(i.e.\ independent of $g_c$, which is the case once Assumption~A is
being verified for a specific candidate), Problem~\ref{prob:core}
reduces to a linear scalar Riemann--Hilbert problem on $(-1,1)$
and can be formally rewritten as follows.
Set $h_c(t) := \tfrac{1}{2}\operatorname{Re}(\phi_c(t))$ for $t\in(-1,1)$.
Then, provided $h_c$ has sufficient regularity on $(-1,1)$
(e.g.\ $h_c \in L^2(-1,1)$) and the finite Hilbert transform
$\mathcal{H}_{(-1,1)}$ is applied in the principal value sense,
a compatible $g_c$ is formally given by
\[
  g_c^+(t) = h_c(t)
  + i\,\mathcal{H}_{(-1,1)}[h_c](t) + iC_c,
\]
where $C_c \in \mathbb{R}$ is determined by the normalisation
$g_c(z) = \log z + O(z^{-1})$ at infinity.
Existence and uniqueness of $g_c$ thus reduce, in principle,
to the invertibility properties of the finite Hilbert transform
on $(-1,1)$ applied to $h_c$, together with the growth condition.
This formulation makes Problem~\ref{prob:core} amenable to the
classical theory of singular integral equations on an interval
(Muskhelishvili~\cite{Muskhelishvili1953}, see also~\cite{Deift1999}).
\end{remark}
...
see Lemma~\ref{lem:contour-reduction} for the explicit derivation.
...
This can be viewed as a scalar Riemann--Hilbert problem with
nonlinear coupling through the real-part constraint: given
$\phi_c$ (Widom phase, which depends on $g_c$ implicitly),
find an analytic function with the prescribed real boundary values
\eqref{eq:g-matching} and the correct normalisation at infinity.
The coupling between $g_c$ and $\phi_c$ renders this genuinely
nonlinear, distinct from the classical linear scalar RHP arising
in the OP setting.
The existence and uniqueness of a solution is an open problem.
\end{proposition}

\begin{lemma}[Leading-order mismatch of the semicircle candidate]
\label{lem:semicircle-mismatch}
Let $g_c^{\mathrm{sc}}(z) = \int_{-1}^1 \log(z-t)\,\frac{\sqrt{1-t^2}}{\pi}\,dt$
...
(the value of the logarithmic potential of the semicircle measure
on $(-1,1)$, see e.g.~\cite{Deift1999} Appendix).
The precise value of $C_0$ is irrelevant for the mismatch argument:
what matters is that $2\operatorname{Re}((g_c^{\mathrm{sc}})^+(t))$
is a fixed polynomial in $t$, independent of $c$.
...
since the integrand $\arccos(s)\,\rho(s)$ is strictly positive
and $O(1)$ on $(t,1)$ for $t < 1$.
...
measure but must track the $O(c)$ growth of the Widom phase.
This confirms that Assumption~A is a genuine open variational problem
involving a $c$-dependent family of analytic functions, not a
verification of a known candidate.
\end{remark}

\begin{remark}[The semicircle candidate]
\label{rem:semicircle-candidate}
The function $g_c^{\mathrm{sc}}$ is motivated by the GUE $g$-function,
but is an import from random matrix theory and is not derived from
the prolate problem.
Lemma~\ref{lem:semicircle-mismatch} shows it fails at leading order
in $c$. A compatible $g_c$ must be constructed directly from
the matching equation \eqref{eq:g-matching}.
\end{remark}
...
scalar Riemann--Hilbert problem with nonlinear coupling described in
Proposition~\ref{prop:g-variational}, and cannot be replaced by
the semicircle candidate $g_c^{\mathrm{sc}}$ (Lemma~\ref{lem:semicircle-mismatch}).
\end{assumption}
...
contains exponential factors $\exp(\pm n(2g_c(z) - \phi_c(z)))$.
The steepest-descent mechanism is governed by
$\operatorname{Re}(2g_c(z) - \phi_c(z))$, not by $g_c$ alone.
The interaction of $g_c$ and $\phi_c$ is the essential difficulty
captured by Assumption~A.
\end{remark}
...
The deep difficulty is Assumption~A; the geometric conditions of
Assumption~B are expected to follow by local analysis once
Assumption~A is established.
\end{remark}
...
Solvability of this scalar RHP with nonlinear coupling
is the central open problem.\\[6pt]
...
\item \emph{Bounded Cauchy operators:}
  ...
  from Assumption~B(ii) and Muckenhoupt-type estimates;
...
existence problem for a scalar Riemann--Hilbert equation with
nonlinear coupling.
...
Find an analytic function $g_c:\mathbb{C}\setminus(-\infty,1]\to\mathbb{C}$
with $g_c(z)=\log z + O(z^{-1})$ satisfying the scalar
Riemann--Hilbert equation with nonlinear coupling
\[
  2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t)),
  \quad t\in(-1,1),
...
By Lemma~\ref{lem:semicircle-mismatch}, the semicircle candidate
$g_c^{\mathrm{sc}}$ fails at leading order in $c$ and cannot serve
as a solution.
The correct $g_c$ must track the $O(c)$ growth of $\phi_c$ and is
genuinely $c$-dependent.
A natural approach is via the finite Hilbert transform formulation
of Remark~\ref{rem:hilbert-formulation}: once $\phi_c$ is known,
the matching equation \eqref{eq:core-problem} can in principle be
analysed using the classical theory of singular integral equations
...
scalar Riemann--Hilbert equation with nonlinear coupling for the
compatible $g$-function (Assumption~A, Problem~\ref{prob:g-existence}).
...
random matrix theory can serve as a solution.
...
in the solvability of this scalar Riemann--Hilbert equation with
nonlinear coupling, whose solvability is currently open.}}
\end{center}
