...
universality holds if and only if there exists a $g$-function
compatible with the Widom edge phase via a nonlinear scalar
Riemann--Hilbert matching condition. Given such a $g$-function,
all steepest-descent geometry (sign condition, Lipschitz contours,
no additional critical points) reduces to a second, purely analytic
assumption.
...
equivalent to a single variational compatibility problem.
\end{abstract}
...
problem to two explicit, logically separated analytic inputs.
...
The main structural result is the following equivalence:

\begin{center}
\fbox{\parbox{0.9\linewidth}{\centering\itshape
Airy universality for the prolate concentration operator holds
if and only if there exists a function $g_c$ satisfying the
variational matching condition
$2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t))$
on $(-1,1)$ with $g_c(z) = \log z + O(z^{-1})$ at infinity.
The existence of such $g_c$ is an open variational problem
...
The present setting differs from random matrix theory in that the
underlying operator is deterministic and arises from time--band
limiting rather than orthogonal polynomial ensembles
(see~\cite{TraWid1994,Deift1999}).
In the classical OP setting, the $g$-function is uniquely determined
as the Stieltjes transform of the equilibrium measure; here it must
be found as the solution of a nonlinear compatibility equation
between two a priori unrelated objects: the semicircle equilibrium
potential and the Widom edge phase.
...
the existence of a compatible $g$-function (structural, open, variational)
and the geometric properties of the steepest-descent contour once such
a $g$-function is given (analytic, conditional).

\subsubsection{The variational $g$-problem (Assumption~A)}

\begin{proposition}[Variational characterisation of the compatible $g$-function]
...
$g_c^+(t)-g_c^-(t) = 2\pi i\int_t^1\rho(s)\,ds$ (cf.\
Proposition~\ref{prop:g-properties}(iii)), condition \eqref{eq:g-matching}
is the \emph{real-part matching condition} required for the
LU-factorisation of the conjugated jump $J_c^T$ to be of
oscillatory off-diagonal form.

This condition constitutes a nonlinear scalar Riemann--Hilbert equation
for $g_c$: given $\phi_c$ (Widom phase, extrinsic to $g_c$), find an
analytic function with the prescribed real boundary values
\eqref{eq:g-matching} and the correct normalisation at infinity.
The existence and uniqueness of a solution to this equation is an
open problem.
\end{proposition}

\begin{remark}[The semicircle candidate]
\label{rem:semicircle-candidate}
The function
\[
  g_c^{\mathrm{sc}}(z) := \int_{-1}^{1} \log(z-t)\,\rho(t)\,dt,
  \quad \rho(t)=\tfrac{1}{\pi}\sqrt{1-t^2},
\]
is a natural candidate, motivated by the fact that it is the unique
$g$-function for the GUE-type ensemble supported on $[-1,1]$.
However, $g_c^{\mathrm{sc}}$ is not derived from the prolate problem:
it is an import from random matrix theory.
Whether $g_c^{\mathrm{sc}}$ satisfies \eqref{eq:g-matching} --- i.e.\
whether the semicircle equilibrium potential correctly reproduces
the Widom edge phase --- is a nontrivial nonlinear compatibility question
and is not assumed here.
The candidate $g_c^{\mathrm{sc}}$ serves as a concrete starting point for
the analysis of Assumption~A, but does not replace it.
\end{remark}
...
it is the solution of the Euler--Lagrange variational problem for
the associated energy functional, and the real-part matching condition
\eqref{eq:g-matching} follows automatically as an identity.
In the present setting, $\phi_c$ is the Widom edge phase, which is
extrinsic to any orthogonal polynomial structure of the prolate operator.
The compatibility condition \eqref{eq:g-matching} is therefore a
genuine open constraint, not an automatic consequence.
This is the structural difficulty that separates the PSWF transition
problem from its random matrix analogues.
\end{remark}
...
satisfying the variational matching condition \eqref{eq:g-matching}:
\[
  2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t)),
  \quad t\in(-1,1),
\]
...
This is a structural assumption. It asserts the solvability of the
nonlinear scalar Riemann--Hilbert equation for $g_c$ described in
Proposition~\ref{prop:g-variational}, and cannot be replaced by
the semicircle candidate $g_c^{\mathrm{sc}}$ without independent verification.
\end{assumption}
...
\[
  \operatorname{Re}\bigl(2g_c^+(t)\bigr) = \operatorname{Re}(\phi_c(t)),
  \quad t\in(-1,1).
\]
\end{enumerate}
\end{proposition}
...
Widom edge phase. The interaction of these two objects is the
essential difficulty captured by Assumption~A.
\end{remark}
...
critical points on $\Sigma_c^\pm \setminus D_\epsilon$, i.e.\
\[
  \nabla_z \operatorname{Re}\!\bigl(2g_c(z) - \phi_c(z)\bigr) \neq 0
  \quad\text{on } \Sigma_c^\pm \setminus D_\epsilon.
\]
This ensures that the only relevant saddle contributing to the
steepest-descent analysis is the $A_2$ coalescing saddle at the
band edge, located at $z=0$ in rescaled coordinates.
All other potential saddle contributions are exponentially suppressed.
\end{enumerate}

Conditions~(i)--(iii) are analytic in nature: given Assumption~A,
they concern the geometry of the level sets of
$\operatorname{Re}(2g_c - \phi_c)$ and are in principle accessible
by local analysis near the edge $z=0$.
\end{assumption}
...
analytic object (a compatible $g$-function) satisfying a nonlinear
boundary condition. Without Assumption~A, Assumption~B cannot even
be formulated, since $2g_c - \phi_c$ is undefined.
...
This separation reflects the actual mathematical structure of the
problem: the deep difficulty is the variational existence problem
(Assumption~A); the geometric conditions (Assumption~B) are
expected to follow by local analysis once Assumption~A is established.
\end{remark}

\begin{remark}[Status of Assumption~B]
\label{rem:g-open}
The additional first-derivative control in condition~(ii) is needed
...
and ensures that the Airy parametrix near $z=0$ accounts for
all relevant contributions to the small-norm error.
\end{remark}
...
More precisely:
\begin{enumerate}[label=(\roman*)]
\item Assumption~A ensures that the conjugated jump $J_c^T$ is of
...
\item Assumption~B(iii) guarantees that no additional critical points
  contribute to the error, so the Airy parametrix
  (Definition~\ref{def:airy-parametrix}) accounts for all relevant
  saddle contributions.
\item Combined with the reconstruction stability of
...
\begin{proof}
Steps~(i)--(iv) follow directly from
...
\end{equation}
This matching requires Assumptions~A and~B
and the explicit phase identification
from Lemma~\ref{lem:edge-normal-form}.
Assumption~B(iii) ensures that the Airy saddle at $z=0$ is the
unique contributing saddle, so the parametrix accounts for
all relevant local behaviour.
\end{remark}
...
\textbf{Assumption~A} (structural / variational):\\
Existence of $g_c$ satisfying
$2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t))$
on $(-1,1)$, $g_c(z)=\log z + O(z^{-1})$.
This is a nonlinear scalar Riemann--Hilbert equation; its
solvability is the central open problem.\\[6pt]
\textbf{Assumption~B} (geometric / analytic, conditional on A):\\
Strict sign $\operatorname{Re}(2g_c-\phi_c) < -\kappa < 0$
on $\Sigma_c^\pm \setminus D_\epsilon$,
uniform Lipschitz lens contours,
...
\item \emph{Bounded Cauchy operators:}
  ...
  Assumption~B(ii), which ensures that the lens
...
\end{itemize}
Both ingredients are supplied by Assumptions~A and~B and
...
lens-opening construction and is the content of Assumption~A.
\end{remark}
...
Assumption~A is the deeper of the two: it is a variational
existence problem for a nonlinear scalar Riemann--Hilbert equation.
Assumption~B is geometric and expected to follow from local
analysis near $z=0$ once Assumption~A is established.
Thus the present work reduces Airy universality for the prolate
operator to a single variational compatibility problem.
\end{remark}
...
solution of an independent variational problem rather than
...
\begin{problem}[Assumption~A: Solvability of the variational $g$-problem]
\label{prob:g-existence}
Find an analytic function $g_c:\mathbb{C}\setminus(-\infty,1]\to\mathbb{C}$
with $g_c(z)=\log z + O(z^{-1})$ satisfying the nonlinear scalar
Riemann--Hilbert equation
\[
  2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t)),
  \quad t\in(-1,1),
\]
for all sufficiently large $c$, where $\phi_c$ is the Widom edge phase.
Establish existence (and, if possible, uniqueness) of such a $g_c$.

The semicircle candidate $g_c^{\mathrm{sc}}$ (Remark~\ref{rem:semicircle-candidate})
provides a concrete starting point: verify whether
$2\operatorname{Re}((g_c^{\mathrm{sc}})^+(t)) = \operatorname{Re}(\phi_c(t))$
holds on $(-1,1)$ for large $c$, either exactly or to leading order.
If not, determine the correct $g_c$ by solving the matching equation
directly, e.g.\ via the theory of scalar Riemann--Hilbert problems
on an interval or by energy-functional methods.

This is the central open problem of the present framework.
\end{problem}
...
Deift--Zhou programme can be initiated at all.
Assumption~B is the analytic completion: given~A, it delivers the
exponential decay required for the small-norm argument.
...
analytic inputs: a structural variational problem (Assumption~A)
and a geometric steepest-descent condition (Assumption~B).
...
nonlinear scalar Riemann--Hilbert equation for the compatible
$g$-function (Assumption~A, Problem~\ref{prob:g-existence}).
This is a genuine variational problem, not a technical estimate:
unlike the classical OP setting, the $g$-function is not determined
by the prolate operator itself but must be found as the solution of
an independent compatibility equation between the equilibrium
potential and the Widom edge phase.
...
logically separated: Assumption~A (structural) must be established
first; Assumption~B (geometric) is then expected to follow by
local analysis near the $A_2$ saddle.
\end{enumerate}

The main result can be stated as the following equivalence:

\begin{center}
\fbox{\parbox{0.9\linewidth}{\centering\itshape
Airy universality for the prolate concentration operator in the
transition regime is equivalent to the existence of a $g$-function
$g_c$ satisfying the variational matching condition
$2\operatorname{Re}(g_c^+(t)) = \operatorname{Re}(\phi_c(t))$
...
in the solvability of this nonlinear scalar Riemann--Hilbert equation.}}
\end{center}
...
The framework suggests that Airy universality is not specific to
random matrix ensembles, but is a structural consequence of integrable
operator representations combined with coalescing saddle geometries,
provided a compatible $g$-function exists.
The variational $g$-problem (Problem~\ref{prob:g-existence}) is the
natural next step toward a fully rigorous proof.
