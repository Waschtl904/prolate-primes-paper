...
\textit{Changes v42\,$\to$\,v43:}
three surgical reviewer-proofing
fixes only (see below);
...
\end{lemma}
\begin{proof}
$\Bc$ and $B_{c'}$ are orthogonal
projections in Fourier space onto
$[-c,c]$ and $[-c',c']$ respectively;
$[-c,c]\subset[-c',c']$ implies
$\Bc\le B_{c'}$ as operators
...
\end{lemma}
\begin{proof}
% Fix F43-1: P*P^2 corrected to P*P
For $f\in C_c^\infty$,
$\|\Hc f - P^*Pf\|
\le\|\Bc P^*Pf - P^*Pf\|\to 0$
as $c\to\infty$
...
\end{theorem}
\begin{proof}
% Fix F43-3: net ordered by c<=c' stated explicitly
Both $\Hstrong$ (Lem.~\ref{lem:hlim-bounded})
and $\overline{\Hspectral}$
(\eqref{IXb}) are SOT limits
of the net $(\Hc)_{c\in(0,\infty)}$
ordered by $c\le c'$,
in the SOT topology on
$\mathcal{B}(L^2)$.
This topology is Hausdorff, so
limits are unique;
...
\begin{remark}
No eigenfunction analysis:
equality follows from categorical
uniqueness in the Hausdorff SOT
topology.
