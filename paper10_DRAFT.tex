...
\textit{Changes v41\,$\to$\,v42:}
global compression pass;
``bounded increasing function
on $(0,\infty)$'' in Def.~\ref{def:qlim};
...
\item\label{IXb}
  ...
  \overline{\Hspectral}$
  (same directed family and SOT
  topology as Lem.~\ref{lem:hlim-bounded}).
\end{enumerate}
...
\end{theorem}
\begin{proof}
Both are SOT limits of $(\Hc)_{c>0}$
in $\mathcal{B}(L^2)$;
the SOT topology on $\mathcal{B}(L^2)$
is Hausdorff, so SOT limits are
unique.
\end{proof}
