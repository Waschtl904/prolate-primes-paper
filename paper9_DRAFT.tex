...
\begin{remark}[Why Hyp.~\ref{hyp:HSOT-discrete} is nontrivial]
...
SOT convergence \eqref{P3} does not imply
norm-resolvent convergence;
...
$|\lambda_n^{(\infty)} a_n^{(k)}
\ip{\Phi_n^{(\infty)}}{\phi}|
\le \lambda_n^{(\infty)}
|\ip{\Phi_n^{(\infty)}}{\phi}|\cdot|a_n^{(k)}|$,
and $\sum_n(\lambda_n^{(\infty)})^2
|\ip{\Phi_n^{(\infty)}}{\phi}|^2<\infty$
(since $\phi\in L^2$ and $\{\lambda_n^{(\infty)}\}$
is bounded).
As $k\to\infty$, $a_n^{(k)}\to 0$ for each~$n$,
so the sum $\to 0$ for all $\phi$.
Hence $g=0$.
\end{proof}
...
\end{equation}
For the remainder: by \eqref{P1} with $f=\Phi_n^{(c)}$
and $\|\Phi_n^{(c)}\|=1$:
\begin{equation}\label{eq:remainder}
  \|r_c\| = \|(\HSOT-\Hc)\Phi_n^{(c)}\|
  \le C_\kappa c^{-3/4} \to 0.
\end{equation}
\eqref{P1} applies here because it holds
...
\begin{align}
  ...
  && (\eqref{P2}\text{ and Thm.~\ref{thm:phi-conv}}).
\end{align}
...
it is fully justified by \eqref{P1}
because \eqref{P1} holds for all unit vectors,
...
The bound \eqref{eq:remainder} uses \eqref{P1},
...
Thm.~\ref{thm:opnorm} is therefore an operator-norm
rate for $\|\Hc-\Hspec\|$, not for $\|\Hc-\HSOT\|$,
