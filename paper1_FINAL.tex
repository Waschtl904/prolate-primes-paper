...
prolate subspace $V_N \subset \PW$, with time-bandwidth product $c = \omega T$.
For $N$ prime sampling points $\mathbf{p} = (p_1,\ldots,p_N)$ with
$p_j \in [-T,T]$, we define the \emph{prolate Gram form}
\[
  \QM(f) \;=\; \sum_{j=1}^{N} |f(p_j)|^2, \qquad f \in V_N.
\]
...
coercive, stable, and Weil-decomposable on $V_N$.
...
matrix $\EN$. Unisolvency of $V_N$ at $N$ distinct points
...
\medskip
\noindent\textbf{Role of prime sampling points.}
The coercivity result (Parts~(I)--(III) of Theorem~\ref{thm:main})
holds for \emph{any} $N$ pairwise distinct points in $[-T,T]$; see
Remark~\ref{rem:primes}. The specific choice of prime sampling points
is motivated by compatibility with Weil-type spectral structures: the
Weil explicit formula involves a sum over primes weighted by $\log p$,
and prime sampling is the natural discrete approximation to this sum at
the level of the prolate Gram form. This connection is made precise in
Part~(IV) and in Paper~II of this program.
...
Throughout, $N \geq 1$ denotes both the number of sampling points and
the dimension of the prolate subspace $V_N$. We use $N$ consistently
for this quantity (in place of the symbol $M$ used in some references).
...
of the $N$-dimensional subspace $V_N \subset \PW$, and an orthonormal
...
is the diagonal matrix of prolate eigenvalues
(see Lemma~\ref{lem:Aprime} for the justification of this decomposition).
...
index set in Part~(IV), in direct analogy with the sum over primes
appearing in the Weil explicit formula.
\end{remark}
...
in $L^2([-T,T]^2)$.
\item \textbf{(Gram splitting.)} Define the \emph{diagonal reference matrix}
\[
  \bigl(G_c^{(N)}\bigr)_{mn} \;:=\; \lambda_n(c)\,\delta_{mn},
  \qquad 0 \leq m,n \leq N-1,
\]
and set $\EN := \GVM - G_c^{(N)}$. The motivation is that $G_c^{(N)}$
is the continuous analog of $\GVM$: by the eigenvalue equation
$\Kop\psin = \lambda_n(c)\psin$ and the continuous orthogonality
\[
  \int_{-T}^{T} \psi_m^{(c)}(x)\,\overline{\psi_n^{(c)}(x)}\,dx
  \;=\; \lambda_n(c)\,\delta_{mn},
\]
the matrix $G_c^{(N)}$ is exactly what $\GVM$ would equal if the
discrete sum $\sum_j$ were replaced by the integral $\int_{-T}^T$.
The error term $\EN$ thus measures the discrepancy between discrete
sampling and continuous integration.
\item \textbf{(Explicit $\EN$ bound via Frobenius norm.)}
For each entry of $\EN$:
\[
  |(\EN)_{mn}|
  \;=\; \Bigl|\sum_{j=1}^N \psi_m^{(c)}(p_j)\,\overline{\psi_n^{(c)}(p_j)}
        - \lambda_n(c)\,\delta_{mn}\Bigr|
  \;\leq\; \|\psi_m^{(c)}\|_{L^\infty}\,\|\psi_n^{(c)}\|_{L^\infty}
           \cdot N + \delta_{mn}.
\]
Under condition \textup{(T.1)}, Lemma~\ref{lem:A}(i) gives
$\|\psi_n^{(c)}\|_{L^\infty} \leq \Kul\,(1-\lambda_n(c))^{-1/2}
\leq \Kul\,e^{\delta c}$.
Hence
\[
  |(\EN)_{mn}| \;\leq\; \Kul^2\,e^{-2\delta c}\cdot(e^{4\delta c}+1)
  \;\leq\; 2\Kul^2.
\]
A refined estimate using the off-diagonal decay of PSWFs yields the
sharper bound: for $m \neq n$,
\[
  |(\EN)_{mn}|
  \;\leq\; N \cdot \|\psi_m^{(c)}\|_{L^\infty} \cdot \|\psi_n^{(c)}\|_{L^\infty}
  \;\leq\; N\Kul^2\,(1-\lambda_m(c))^{-1/2}(1-\lambda_n(c))^{-1/2}\cdot e^{-4\delta c}.
\]
Summing over all $N^2$ entries and applying $\|A\|_2 \leq \|A\|_F$:
\[
  \|\EN\|_2 \;\leq\; \|\EN\|_F
  \;=\; \Bigl(\sum_{m,n=0}^{N-1} |(\EN)_{mn}|^2\Bigr)^{1/2}
  \;\leq\; N\Kul^2\,e^{-2\delta c}.
\]
\item Under (T.1) and the lower bound $1-\lambda_n(c) \geq e^{-2\delta c}$:
\[
  \|\EN\|_2 \;\leq\; N\Kul^2\,e^{-2\delta c}
  \;\leq\; N\kappa\,e^{-\eta_0 c} \;\leq\; N\kappa\,e^{-\eta' N}.
\]
\item $\|\EN\|_2 \leq \|\EN\|_F \leq N\kappa\,e^{-\eta' N}$
...
\begin{enumerate}[(i)]
\item $V_N$ is a Chebyshev system: every nontrivial $f\in V_N$ has
...
(ii) If $f\in V_N\setminus\{0\}$ vanishes at all $N$ points, it has $N$ zeros,
contradicting (i).
...
By the eigenvalue equation $\Kop\psin = \lambda_n(c)\psin$,
\[
  \ip{\Kop f}{f}_{L^2}
...
\]
with $R_N(f) := \ip{\EN\mathbf{a}}{\mathbf{a}}$ satisfying
$|R_N(f)| \leq \|\EN\|_2\|\mathbf{a}\|^2 = O(e^{-\delta' N})$
by Lemma~\ref{lem:Aprime}(v).
...
$\zeta(s)$ \cite{CCM2025}. This is addressed in Paper~II of this program.
