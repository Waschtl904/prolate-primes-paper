...
\begin{remark}[Two proof levels: decay vs.\ rate]
\label{rem:two-levels}
This paper operates on two distinct levels, and it is important
not to conflate them:
\begin{enumerate}[(I)]
\item \textbf{(Existence of decay, rigorous)}
The core result --- $\Omega_\varepsilon(c) \to 0$ --- follows
rigorously from the barrier estimate alone
(Theorem~\ref{thm:boundary-decay} and
Corollary~\ref{cor:assumption-B}).
This is self-contained and independent of Paper~VI.
In particular, Assumption~(B) of Paper~IV is established
as a theorem unconditionally for PSWF observables.
\item \textbf{(Rate of decay, analytically supported)}
The sharper claim $\Omega_\varepsilon(c) = O(c^{-1/2})$
rests on stationary-phase analysis
(Proposition~\ref{prop:mechanism-chain}),
which is not yet fully rigorous in this paper.
The rate $\beta = \tfrac{1}{2}$ is consistent with all
numerical evidence (Section~\ref{sec:numerical}) and
is established rigorously in Paper~VI~\cite{PaperVI}.
\end{enumerate}
\emph{Paper~VI sharpens the result of this paper;
