...
$x \in [-c^{1/6}, c^{1/6}]$,
since $\alpha_2 = O(c^{-1/4})$ and $M$ is fixed.
...
$O(c^{-1/6})$ from Lemma~\ref{lem:remainder}.
\end{proof}
...
+ \text{(Jacobian)}
= \frac{O(c^{-2/3})}{O(c^{-1/2})}
+ \frac{O(c^{-1/2}M^{-1})}{O(c^{-1/2})}
+ O(c^{-1/4})
= O(c^{-1/6}) + O(M^{-1}) + O(c^{-1/4}).
\]
For fixed $M$, the dominant term is $O(c^{-1/6})$ (outer zone).
The final relative error in \eqref{eq:composite-A}
is therefore $O(c^{-1/6})$, uniform in $n \le \kappa c$.
\end{remark}
...
The bound $O(c^{-1/4})$ is not sharp
(a more careful argument via stationary phase
in the algebraic zone could yield $O(c^{-1/2+\varepsilon})$),
but it is more than sufficient:
only $J_2 \to 0$ is needed for the conclusion.
