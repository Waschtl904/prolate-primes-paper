...
The crossover zone $\Zcross$ is not merely a
transition artifact between two asymptotic regimes.
It is the region where the correct scaling and
structure of $\widehat{\Phi}_n^{(c)}$ are determined.
The $c^{-1/2}$ decay is not accidental: it emerges
from a cubic stationary-phase mechanism (encoded in the
Jacobian of the Langer transformation), and cannot be
captured by either the Airy or WKB regimes alone.
More precisely, the renormalized profile
\[
  c^{1/2}\,\widehat{\Phi}_n^{(c)}\!\bigl(t_* + c^{-1/2}x\bigr)
\]
admits a uniform representation in terms of
$\mathcal{A}_n^{(c)}(x)$ for $x = O(1)$.
This identifies $\mathcal{A}_n^{(c)}$ as the
\emph{universal profile} of the transform near the
saddle frequency, and the crossover zone as its
natural domain of validity.
...
\begin{lemma}[Uniform zero spacing of the profile]
\label{lem:zero-spacing}
There exists a constant $L_0 = L_0(\kappa) > 0$ such that
for all $c \ge c_0(\kappa)$, all $n \le \kappa c$,
and all $x_0 \in \mathbb{R}$,
there exists $x_1 \in [x_0, x_0 + L_0]$ with
\[
  \mathcal{A}_n^{(c)}(x_1) \ne 0.
\]
\end{lemma}
...
\begin{proof}
The zeros of $\mathcal{A}_n^{(c)}(x)$ coincide with those
of $\Ai(\xi)$ under the scaling
$\xi = x/(\mu_n^{(c)})^{2/3}$.
Since $\mu_n^{(c)} \in [c_\kappa, C_\kappa]$
(Lemma~\ref{lem:uniform-n}),
the spacing between consecutive zeros of
$\mathcal{A}_n^{(c)}$ is bounded by
$C_\kappa^{2/3}$ times the maximal spacing between
zeros of $\Ai$.
By standard Airy asymptotics, this spacing is uniformly
bounded by a universal constant (e.g.\ $\le 5$),
which yields the claim with $L_0 = 5C_\kappa^{2/3}$.
\end{proof}

\begin{theorem}[Uniform composite representation in the crossover regime]
\label{thm:A}
Fix $\kappa \in (0,2/\pi)$.
...
such that $|\mathcal{A}_n^{(c)}(x_1)| \ge c_\kappa(1+|x_1|)^{-1/4}$.
This follows from the standard lower envelope of $|\Ai|$ between its zeros
(\cite{Olver1974}, Ch.~11): between consecutive zeros $|\Ai|$ satisfies
$|\Ai(\xi)| \ge c(1+|\xi|)^{-1/4}$ on a set of positive measure.
Moreover, $L_0$ can be chosen uniformly as
$L_0 = C_\kappa^{2/3}\,L_{\Ai}$,
where $L_{\Ai}$ is the maximal spacing between consecutive zeros of $\Ai$.
Since $\mu_n^{(c)} \in [c_\kappa, C_\kappa]$
(Lemma~\ref{lem:uniform-n}),
...
\begin{remark}[Scope of Theorem~A]\label{rem:thmA-scope}
Equation~\eqref{eq:composite-A} is a \emph{representation theorem},
not an asymptotic expansion in the classical sense
(where the error is $o$ of the main term).
The relative error $O(c^{-1/6})$ in~\eqref{eq:composite-A}
refers to the outer-zone contribution \emph{given} the
coefficient $\kappa_{n,c}$; the coefficient itself is determined
only up to an $O(1)$ factor (Remark~\ref{rem:A-explicit}).
Theorem~A asserts the existence of a profile function
$\mathcal{A}_{n,c}$ satisfying the stated bounds~(a)--(d);
it does not identify $\kappa_{n,c}$ to leading order.
For Theorem~B, only the upper bound~(a) is used,
so this limitation is immaterial.
\end{remark}
...
(Lemma~\ref{lem:uniform-n}).
\begin{remark*}
Here $p''(\ustar;c)$ denotes the coefficient
of $\tfrac{1}{6}s^3$ in the Taylor expansion
of the rescaled phase
$\Psi(s,x) = xs + \tfrac{\alpha_2}{2}s^2
+ \tfrac{\mu_n^{(c)}}{6}s^3 + O(c^{-1/2}s^4)$
(Lemma~\ref{lem:phase}, eq.~\eqref{eq:phase-expand}),
\emph{not} the classical pointwise second
derivative of $\sqrt{-V_c}$,
which diverges at $\ustar$.
The identification $p''(\ustar) \asymp c^{1/2}$
is established in Lemma~\ref{lem:uniform-n}.
\end{remark*}
...
\begin{remark*}
The formula~\eqref{eq:kappa-explicit} is
\emph{not asymptotically resolved at the present error scale}:
the tail contribution of Lemma~\ref{lem:tail-ext}
is $O(c^{-1/2})$, the same order as the leading term,
and modifies $\kappa_n^{(c)}$ by an $O(1)$ factor
absorbed into the implicit constant of Theorem~A.
The coefficient $\kappa_n^{(c)}$ is therefore determined
only up to an $O(1)$ factor at this scale;
a finer resolution would require working at error order
$o(c^{-1/2})$, outside the scope of this paper.
The bound $|\kappa_n^{(c)}| \asymp 1$ is all that
Theorem~A and Theorem~B require.
\end{remark*}
\end{remark}
...
\begin{proof}
At the turning point $V_c(\ustar) = 0$,
so $p(u) = \sqrt{-V_c(u)}$ satisfies
$p'(u)^2 = V_c'(u)^2 / (4(-V_c(u)))$.
L'H\^opital's rule applied as $u \to \ustar$ gives
\[
  \lim_{u \to \ustar} p'(u)^2
  = \lim_{u \to \ustar}
    \frac{(V_c'(u))^2}{-4\,V_c(u)}
  = \frac{2\,V_c'(\ustar)^2}{-4\,V_c'(\ustar)}
  = -\tfrac{1}{2}V_c'(\ustar).
\]
Since $V_c'(\ustar) < 0$
(the potential crosses zero from below),
this gives $p'(\ustar)^2 = \tfrac{1}{2}|V_c'(\ustar)|$.
...
\[
  V_c'(\ustar)
  = 4c^2 e^{4\ustar} - 2\chi_n^{(c)} e^{2\ustar}
  = 2e^{2\ustar}(2c^2 e^{2\ustar} - \chi_n^{(c)})
  \asymp -c^{1/2},
\]
(since $e^{2\ustar} \asymp c^{-1/2}$),
hence $p'(\ustar) \asymp c^{1/4}$
and $p''(\ustar) \asymp c^{1/2}$
(cubic coefficient in the sense of~\eqref{eq:mu-def}),
...
We begin with the explicit form of the Fourier--Mellin transform:
\[
  \widehat{\Phi}_n^{(c)}(t)
  = \int_{\mathbb{R}} \Phi_n^{(c)}(u)\,e^{-2\pi i t u}\,du,
\]
where $\Phi_n^{(c)}$ is the rescaled PSWF (Section~\ref{sec:setup}).
We decompose the Fourier--Mellin transform.
All estimates below are uniform for $n \le \kappa c$
and $c \ge c_0(\kappa)$, unless stated otherwise.
...
\[
  ...
  \bigr],
\]
where the three integrals are over the disjoint regions
$\mathcal{Z}_{\mathrm{inn}} := \{|s| \le M\}$,
$\mathcal{Z}_{\mathrm{mid}} := \{M < |s| \le c^{1/6}\}$,
and $\mathcal{Z}_{\mathrm{out}} := \{|s| > c^{1/6}\}$,
which partition $\mathbb{R}$.
The threshold $M = c^{1/12}$ satisfies $M \ll c^{1/6}$ for all large $c$.

\medskip
\noindent\textbf{Step~2: Outer zone.}
On $\mathcal{Z}_{\mathrm{out}} := \{|s| > c^{1/6}\}$,
the phase derivative satisfies
\[
  |\varphi'(s)| = \bigl|x + \tfrac{\mu_n^{(c)}}{2}s^2\bigr|
  \ge \tfrac{c_\kappa}{4}s^2 \ge \tfrac{c_\kappa}{4}c^{1/3},
\]
uniformly for $|x| \le c^{1/6}$ and $n \le \kappa c$
...
\[
  ...
  = O(c^{-2/3}),
\]
which is $O(c^{-1/6})$ relative to the main term $c^{-1/2}$.
...
\[
  ...
  = \kappa_n^{(c)}
    ...
    \cdot(1 + O(c^{-1/4})).
\]
On the inner zone $|s'| \le M$ we approximate $a(s')$ by $a(0)$:
\[
  \int_{|s'|\le M} e^{i(x's'+\mu_n^{(c)}s'^3/6)}\,a(s')\,ds'
  = a(0)\int_{|s'|\le M} e^{i(x's'+\mu_n^{(c)}s'^3/6)}\,ds'
  + O\!\bigl(M^2 \cdot \sup_{|s'|\le M}|a'(s')|\bigr).
\]
Since $\sup|a'| \le C_\kappa c^{-1/4}$
(Lemma~\ref{lem:amp-near}\,(ii)) and $M = c^{1/12}$,
the error is $O(M^2 c^{-1/4}) = O(c^{-1/12})$.
This identifies $a(0) = \kappa_n^{(c)}$
from \eqref{VI4}; the error is absorbed into the $O(1)$ ambiguity
of $\kappa_n^{(c)}$ already noted in Remark~\ref{rem:A-explicit}.
By Lemma~\ref{lem:tail-ext} with $M = c^{1/12}$,
extending the inner integral to $\mathbb{R}$ introduces error
$O(c^{-1/3}M^{-2}) = O(c^{-1/2})$.
This is of the same order as the principal term,
and prevents identification of the leading coefficient at scale $c^{-1/2}$.
The coefficient $\kappa_n^{(c)}$ is therefore determined only up to an
$O(1)$ factor; this is recorded in Remark~\ref{rem:A-explicit}
and is all that Theorem~A and Theorem~B require.
This shows that the truncation error affects only the
multiplicative constant $\kappa_n^{(c)}$,
not the scaling $c^{-1/2}$.
%%═══════════════════════════════════════════
...
Collecting contributions from Steps~2--4:
\[
  \widehat{\Phi}_n^{(c)}(t)
  = c^{-1/2}\,\mathcal{A}_n^{(c)}(x)
  + \underbrace{O(c^{-2/3})}_{\text{outer (Step~2)}}
  + \underbrace{O(c^{-13/12})}_{\text{middle (Step~3)}}.
\]
Since $c^{-2/3} = c^{-1/2}\cdot c^{-1/6}$ and
$c^{-13/12} = c^{-1/2}\cdot c^{-7/12}$,
both errors are $O(c^{-1/6})$ relative to $c^{-1/2}$, giving
\[
  \widehat{\Phi}_n^{(c)}(t)
  = c^{-1/2}\,\mathcal{A}_n^{(c)}(x)
...
\begin{itemize}
  ...
  \item[(b)] Between consecutive zeros of $\Ai$,
        the standard lower envelope gives
        $|\Ai(\xi)| \ge c_0 (1+|\xi|)^{-1/4}$
        on a set of positive measure
        (\cite{Olver1974}, Ch.~11, eq.~(11.11.1)).
        Scaling by $\mu_n^{(c)} \in [c_\kappa, C_\kappa]$
        (Lemma~\ref{lem:uniform-n})
        yields $|\mathcal{A}_n^{(c)}(x_1)| \ge c_\kappa(1+|x_1|)^{-1/4}$.
        The uniform zero spacing bound $L_0 = C_\kappa^{2/3}L_{\Ai}$
        follows from Lemma~\ref{lem:zero-spacing}.
...
\textit{Determination of $c_3(\kappa,\varepsilon)$.}
The threshold $c_3(\kappa,\varepsilon)$ is the value at which
$J_2 \le \varepsilon^2/3$.
Since $J_2 = O(c^{-1/4})$, there exists an absolute constant
$A_\kappa > 0$ such that $J_2 \le A_\kappa c^{-1/4}$.
Setting $A_\kappa c^{-1/4} \le \varepsilon^2/3$ gives the
explicit threshold:
\[
  c_3(\kappa,\varepsilon)
  := \Bigl(\frac{3A_\kappa}{\varepsilon^2}\Bigr)^4.
\]
The overall threshold is then
$c_0(\kappa,\varepsilon)
:= \max\bigl(c_1(\kappa,\varepsilon),\,
              c_2(\kappa,\varepsilon),\,
              c_3(\kappa,\varepsilon)\bigr)$,
which is finite for all
$\kappa \in (0,2/\pi)$ and $\varepsilon \in (0,1)$.
\end{proof}
...
far from $t_*$.
The key obstruction is not merely that the mass argument
yields the wrong order $c^{-1}$ instead of $c^{-1/2}$;
it is deeper: the mass argument is
\emph{insensitive to the relevant localisation scale}.
Integrating $|\widehat{\Phi}_n^{(c)}|^2$ over regions
far from $t_*$ cannot distinguish constructive
concentration from destructive cancellation near $t_*$.
In other words, the argument is invariant with respect
to the localisation scale that determines
$\Delta_\varepsilon$, and therefore cannot produce
a peak-sharp lower bound at any scale.
The best achievable bound via $\mathcal{M}$ is
$\Delta_\varepsilon(c,n) \ge c_1(\kappa,\varepsilon)\,c^{-1}$,
weaker than the conjectured $c^{-1/2}$ by a factor $c^{1/2}$.
Any proof of $\Delta_\varepsilon \ge c_1 c^{-1/2}$
must therefore establish
\emph{pointwise non-cancellation} of
$\widehat{\Phi}_n^{(c)}$ near $t_*$
(Problem~\ref{prob:nonvanish}; Paper~VIII):
a fundamentally different type of estimate,
outside the methodological scope of this paper.
\end{proof}
