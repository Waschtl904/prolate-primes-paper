...
The coefficient $\kappa_n^{(c)}$ is defined by matching
\eqref{eq:composite-A} with the Fourier--Airy profile
\eqref{VI4} at $t = \tstar$ (i.e.\ $x = 0$), giving
...
\begin{enumerate}[\rm(i)]
  \item $\|a\|_{L^\infty} \le C_\kappa\,c^{-1/2}$,
        uniformly in $n \le \kappa c$.
...
\begin{proof}
\textit{(i)} Follows from the uniform Airy approximation
\eqref{VI1} with $|D_n^{(c)}| \asymp c^{1/4}$
and $|p(\ustar)|^{-1/6} \asymp c^{-1/12}$,
giving $|\Phi_n^{(c)}| = O(c^{1/4} \cdot c^{-1/12}) = O(c^{1/6})$;
after the $c^{-1/2}$ Jacobian factor, $\|a\|_{L^\infty} = O(c^{-1/2})$.
...
$|a/\varphi'|_{c^{1/6}} \le c^{-1/2}\cdot c^{-1/3} = c^{-5/6}$
and remainder integral
$\int_{c^{1/6}}^\infty c^{-1/3}s^{-2}\,ds
= c^{-1/3}\cdot c^{-1/6} = c^{-1/2}$,
which is $O(c^{-1/3})$ relative to $c^{-1/2}$.
\end{proof}
...
\]
In particular, this bound is
$o(c^{-5/6})$ for any fixed $M \ge 1$
and $c \to \infty$.
\end{lemma}
...
At $|s'| = M$:
$|\tilde{a}/\tilde\varphi'|
\le C_\kappa c^{-1/2} \cdot \tfrac{4}{c_\kappa}M^{-2}
= O(c^{-1/2}M^{-2})$.
At $|s'| = c^{1/6}$:
$|\tilde\varphi'| \gtrsim c^{1/3}$,
so $|\tilde{a}/\tilde\varphi'| = O(c^{-5/6})$.
...
\[
  ...
  + \frac{C_\kappa c^{-1/2} \cdot C_\kappa c^{1/6}}{(c_\kappa M^2/4)^2}
  = O(c^{-1/3}M^{-2}).
\]
...
\textit{Total and conversion to stated bound.}
Combining boundary and remainder:
$O(c^{-1/2}M^{-2}) + O(c^{-1/6}M^{-2})
= O(c^{-1/6}M^{-2})$.
Since $M$ is fixed and $c \ge c_0(\kappa,M)$,
we have $c^{1/6} \ge 1$, hence
\begin{equation}
\label{eq:intermediate-inequality}
  c^{-1/6}M^{-2} \;\le\; c^{-1/2}M^{-1}
  \;\iff\;
  M \;\le\; c^{1/3},
\end{equation}
which holds for all fixed $M$ and $c \ge M^3$.
Thus the total is $O(c^{-1/2}M^{-1})$, as claimed.
\end{proof}
...
\[
  ...
  = O(c^{-1}M^{-1}).
\]
This is $O(c^{-1/3})$ relative to $c^{-1/2}$
for any fixed $M$.
...
which contributes $O(c^{-1/4})$ to the relative error):
\[
  \mathcal{I}_{\mathrm{inn}}
...
The dominant error is $O(c^{-1/2})\cdot O(c^{-1/3})
= O(c^{-5/6})$ absolutely, i.e.\ $O(c^{-1/3})$
