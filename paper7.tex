...
\item \textbf{Locality of the growing window:}
Although $|s|, |x| \le c^{1/6} \to \infty$,
the window remains \emph{locally small} relative
to the natural turning-point scale $c^{1/4}$:
\[
\frac{c^{1/6}}{c^{1/4}} = c^{-1/12} \to 0.
\]
Since $\mu_n^{(c)} \in [c_\kappa, C_\kappa]$ uniformly
(Lemma~\ref{lem:uniform-n}), the normal-form
transformation depends on coefficients that remain
uniformly bounded for $n \le \kappa c$.
In particular, the Olver diffeomorphism
$s \mapsto s'(s,x)$ is well-defined and smooth on
$|s| \le c^{1/6}$, and its Taylor expansion yields
a remainder $O(c^{-1/3})$ uniformly in this window.
Hence the classical Olver theorem
(\cite{Olver1974}, Ch.~11, Thm.~11.1)
applies uniformly on the growing window
$|s| \le c^{1/6}$ for all $c \ge c_0(\kappa)$
and $n \le \kappa c$.
\end{enumerate}
\end{proof}
...
\begin{remark*}
The bound $|a'(s)| = O(c^{-1/3})$ is used in
Lemma~\ref{lem:remainder} in the form
$|a'(s)|/|\varphi'(s)| \lesssim c^{-1/3} \cdot s^{-2}$;
since $|s| \ge c^{1/6}$ in the tail region,
this is $\le c^{-1/3} \cdot c^{-1/3} = c^{-2/3}$,
which is consistent with the $O(c^{-1/3})$ relative error
in \eqref{eq:composite-A}.
\end{remark*}
\end{proof}
...
Both contributions are $O(c^{-5/6})$ absolutely
(boundary term) respectively $O(c^{-1/2})$
(remainder integral, matching the main term order),
...
$J_1 \le 2C_\kappa^2\,c^{-17/12}$ by
Proposition~\ref{prop:cross-mass}.
Set $\Delta = Kc^{-1/2}$ and choose
$K = (8C_\kappa^2/\varepsilon^2)^2$ so that
$J_2 \le \varepsilon^2/2$;
then $J_1+J_3 \le \varepsilon^2/6$ for large $c$.
\end{proof}
...
Note that using the sharper algebraic bound
$|\widehat{\Phi}_n^{(c)}(t)| \le C_\kappa c^{-1/4} t^{-1/4}$
near $\tstar$ (where $t \asymp c^{1/2}$,
giving $|\widehat{\Phi}| \le C_\kappa c^{-1/2}$)
does not improve this scaling:
the bottleneck is the $L^2$-mass budget,
not the pointwise amplitude.
\end{proof}

\begin{remark}[Structural obstruction]
\label{rem:structural}
This shows that the obstruction to the lower bound
is not technical but structural:
any method that cannot distinguish pointwise
non-cancellation from cancellation
will necessarily fail to recover the $c^{-1/2}$ scale.
\end{remark}
...
with $\|\Hc\| \le \tfrac{N(c)}{2} \le \tfrac{\kappa c}{2}$
(using $\|\widehat{\Phi}_n^{(c)}\|^2 = \tfrac{1}{2}$
and $\lambda_n^{(c)} \le 1$).
\end{definition}
...
|a_n^{(c)}|^2
\|\widehat{\Phi}_n^{(c)}\|^2\\
&\le \tfrac{1}{2}\cdot C_\kappa^2 c^{-3/2}
\sum_n |a_n^{(c)}|^2
\le \tfrac{1}{2}C_\kappa^2 c^{-3/2}\|f\|^2.
\end{align*}
