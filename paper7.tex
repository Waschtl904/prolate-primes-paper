...
$\phi'(s) = x + \tfrac{\mu_n^{(c)}}{2}s^2$.
For $|s| \ge c^{1/6}$ and $t \in \Zcross$
(so $|x| \le c^{1/6}$),
the dominant term satisfies
$|\tfrac{\mu}{2}s^2| \asymp s^2 \ge c^{1/3}$,
hence $|\phi'(s)| \asymp s^2 \ge c^{1/3}$.
A single integration by parts gives
\[
\Bigl|\int_{|s|>c^{1/6}}
e^{i(xs + \mu_n^{(c)}s^3/6)}\,ds\Bigr|
\le \frac{C}{\inf_{|s|>c^{1/6}}|\phi'(s)|}
\asymp c^{-1/3}.
\]
Including the $c^{-1/2}$ prefactor from the
amplitude gives $R_{\rm far} = O(c^{-1/2-1/3})
= O(c^{-5/6})$,
which is $O(c^{-1/3})$ relative to the main term
$c^{-1/2}$.
\end{proof}
