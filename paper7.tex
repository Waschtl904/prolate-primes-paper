...
%% ══════════════════════════════════════════════════════════════════
%% FIX: Lemma lem:amp-near — corrected Ai' bound throughout
%% ══════════════════════════════════════════════════════════════════
...
\[
  ...
  |\Ai'(\zeta)| \le C(1+|\zeta|)^{1/4},
\]
we note that for $|u - \ustar| \le c^{-1/3}$
...
requires only $|a'| \to 0$, and $c^{-1/4} \to 0$.
The relative error in Theorem~A remains $O(c^{-1/3})$
as the dominant contribution comes from the outer zone
($c^{-1/6}$) and the Jacobian ($c^{-1/4}$),
not from this bound directly.
\end{remark*}
\end{proof}
...
\label{lem:tail-ext}
For any fixed $M > 1$ \textup{(}independent of $c$\textup{)},
$t \in \Zcross$,
...
Since $M$ is fixed \textup{(}independent of $c$\textup{)},
$c^{-1/3}M^{-2} = o(c^{-1/2})$,
so extending the inner-zone
...
(valid for $c \ge c_0(\kappa,M)$, $M$ fixed).
...
\[
  ...
  = O(c^{-1}M^{-1}).
\]
Relative to $c^{-1/2}$, this is $O(c^{-1/2}M^{-1})$,
which for fixed $M$ is $o(1)$.
...
\[
  ...
  = \kappa_n^{(c)}
    ...
    \cdot(1 + O(c^{-1/4})).
\]
By Lemma~\ref{lem:tail-ext} (with $M$ fixed,
independent of $c$), extending to $\mathbb{R}$
...
%% ══════════════════════════════════════════════════════════════════
%% FIX: Step 4 — corrected J_2 estimate using split + (VI.3)
%% ══════════════════════════════════════════════════════════════════
\textbf{Step~4: Bound on $J_2$ (corrected).}
We split $J_2 = J_2^- + J_2^+$, where
$J_2^-$ integrates over $\{0 < t < t_* - \Delta\}$
and $J_2^+$ integrates over $\{t > t_* + \Delta\}$.

\textit{Bound on $J_2^-$.}
Since $t \le t_*$ throughout this region,
\eqref{VI2} gives
...
\textit{Bound on $J_2^+$.}
Since $t > t_* + \Delta > t_*/2$ for large $c$,
apply \eqref{VI3} with $R = t_* + \Delta \asymp c^{1/2}$
(note $R \le c^{1/4}$ is satisfied for large $c$):
\[
J_2^+
\le \|\widehat{\Phi}_n^{(c)}\|^2_{L^2(t > t_*+\Delta)}
\le C_\kappa^2\,c^{-1/2}\,(t_*+\Delta)^{-1/2}
\asymp C_\kappa^2\,c^{-3/4}.
\]
In particular, $J_2^+ = O(c^{-3/4}) \to 0$,
independently of $K$.

\textit{Combined.}
$J_2 = J_2^- + J_2^+ = O(c^{-1/4}) \to 0$.
Choose $c \ge c_3(\kappa,\varepsilon)$ large enough
so that $J_2 \le \varepsilon^2/3$.
Note that $K$ does not appear in this bound;
it is determined solely by $J_1$ via Step~2.
...
%% ══════════════════════════════════════════════════════════════════
%% FIX: Remark rem:explicit-const — corrected constant scaling
%% ══════════════════════════════════════════════════════════════════
