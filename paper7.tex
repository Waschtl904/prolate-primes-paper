...
elaborates a smooth diffeomorphism $s \mapsto s'$
...
Let $a(s)$ denote the (smooth, bounded) amplitude
arising from $\Phi_n^{(c)}$ after the substitution.
...
Since $|a'(s)| = O(1)$ and $|\varphi'(s)| \gtrsim s^2$,
\[
\left|\frac{d}{ds}\left(\frac{a(s)}{\varphi'(s)}\right)\right|
\lesssim \frac{|a'(s)|}{|\varphi'(s)|}
+ |a(s)| \frac{|\varphi''(s)|}{|\varphi'(s)|^2}
\lesssim s^{-2} + s^{-3} \lesssim s^{-2}.
\]
Hence
\[
\int_{|s|\ge c^{1/6}}
\left|\frac{d}{ds}\!\left(\frac{a(s)}{\varphi'(s)}\right)\right|ds
\lesssim \int_{c^{1/6}}^{\infty} s^{-2}\,ds
\lesssim c^{-1/6}.
\]

Both contributions are $O(c^{-1/6})$ in the rescaled
variable. Including the $c^{-1/2}$ prefactor from the
amplitude gives $R_{\rm far} = O(c^{-1/2} \cdot c^{-1/3})
= O(c^{-5/6})$, which is $O(c^{-1/3})$ relative
to the main term $c^{-1/2}$.
\end{proof}
