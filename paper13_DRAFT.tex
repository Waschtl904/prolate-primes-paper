...
The only hypothesis in the paper strictly weaker than
...
up to normalisation).
Conversely, one can construct examples where
a complete ONS converges SOT to an incomplete limit.
...
\item The only hypothesis in this paper strictly weaker
  than completeness and grounded in the series is~H1.
...
\begin{remark}[H1 is the only strictly weaker grounded hypothesis]
\label{rem:H1-status}
...
if true for all $c_0$).
H1 is therefore the unique hypothesis in this paper
that is:
\begin{itemize}
...
H1 & Coefficient stability
  & $<$ completeness
  & Only grounded weaker hypothesis \\
...
For some (hence: separately for each) fixed $c_0>0$,
...
with H1 as the only currently grounded weaker step.
