...
\medskip
\noindent\textit{Step~1: Decomposition of $I_+$.}
Fix $\delta = c^{-1/3}$, which matches the natural Airy scaling
near the turning point.
Split
$I_+(t,c) = I_+^{\rm loc}(t,c) + I_+^{\rm tail}(t,c)$,
where $I_+^{\rm loc}$ integrates over $|u - u^*(t,c)| \le \delta$
(saddle neighbourhood) and $I_+^{\rm tail}$ over the complement.

\medskip
\noindent\textit{Step~2: Tail estimate via integration by parts.}
On $|u - u^*| > \delta$ the phase $\phi_+(u)$ has no stationary
point, so integration by parts gives
\[
|I_+^{\rm tail}(t,c)| \;\lesssim\; c^{-1/4}\,t^{-1},
\]
uniformly in the stated range of $t$
(using the WKB amplitude bound of Lemma~\ref{lem:amplitude-saddle}(iii)).

\medskip
\noindent\textit{Step~3: Local estimate via Airy matching.}
Near $u^*$ the phase degeneracy prevents standard stationary phase
from recovering the correct $t^{-1/4}$ scaling
(Lemma~\ref{lem:amplitude-saddle}).
The uniform Airy approximation (Lemma~\ref{lem:Langer}) converts
$I_+^{\rm loc}$ into an oscillatory integral with Airy kernel,
whose oscillatory structure yields a decay rate of order $t^{-1/4}$
via stationary phase for cubic-type phases
(see Olver~\cite{Olver1974}, Ch.~11).
...
Each $\Phi_n^{(c)}$ exhibits exponential decay as $u \to -\infty$
due to the potential barrier in~\eqref{eq:Schrodinger},
so that $\Phi_n^{(c)} \in L^1(e^{a|u|}\,du)$ for some $a > 0$.
Consequently, $\widehat{g}_n^{(c)}$ extends to a real-analytic
function on $\mathbb{R}$.

Since the $\psi_n^{(c)}$ form a Sturm--Liouville eigenbasis
with distinct eigenvalues $\chi_n^{(c)}$, the functions
$\{\Phi_n^{(c)}\}$ are linearly independent in $L^2(-\infty,0)$,
and hence their Fourier transforms $\{\widehat{g}_n^{(c)}\}$
are linearly independent on $\mathbb{R}$.

Suppose $F_c(t) = \sum_n \lambda_n^{(c)}\widehat{g}_n^{(c)}(t)$
vanishes identically on a compact $K \subset \mathbb{R}$
with an accumulation point.
By real-analyticity, $F_c \equiv 0$ on all of $\mathbb{R}$.
Taking inverse Fourier transforms:
\[
\sum_n \lambda_n^{(c)} \Phi_n^{(c)}(u) = 0
\quad \text{in } L^2(-\infty,0).
\]
By linear independence of $\{\Phi_n^{(c)}\}$ and
$\lambda_n^{(c)} > 0$, this is a contradiction.
\end{proof}
